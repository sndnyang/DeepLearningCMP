...
def pool2d_layer(inputs, pool_type, pool_size=2, pool_stride=2):
    ...
    if pool_type == "max":
        # Max pooling layer
        return tf.layers.max_pooling2d(inputs, pool_size=[pool_size] * 2, strides=pool_stride)

    elif pool_type == "average":
        # Average pooling layer
        return tf.layers.average_pooling2d(inputs, pool_size=[pool_size] * 2, strides=pool_stride)
...
class CNN(object):
    ...
    def __call__(self, images, is_training=False, update_batch_stats=True, stochastic=True, seed=1234):
        ...
        for i, filter_size in enumerate(self.filter_sizes_conv_layers):
            ...
            with tf.variable_scope(os.path.join("mnist_network", "conv_" + layer_suffix), reuse=reuse):
